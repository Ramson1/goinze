...
import { lecturerApi, type NotificationRecord } from '@/lib/api';
...
export default function NotificationsPage() {
  ...
  async function markRead(n: NotificationRecord) {
    ...
    try {
      await lecturerApi.markNotificationRead(n.id);
    } catch {
  ...
  async function markAllRead() {
    ...
    await Promise.allSettled(targets.map((n) => lecturerApi.markNotificationRead(n.id)));
    setMarkingAll(false);
  }
  ...
  return (
    <div className="mx-auto max-w-3xl">
      <PageHeader
        title="Notifications"
        subtitle="Stay up to date with exam submissions, results, and academic activity."
        actions={
          ...
          {list.map((n) => {
            ...
            return (
              <div
                ...
                title={read ? undefined : 'Click to mark as read'}
              >
                <Card
