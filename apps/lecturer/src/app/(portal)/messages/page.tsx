...
export default function MessagesPage() {
      ...
      .then((convs) => {
        ...
        if (!currentUserId && convs.length > 0) {
          ...
          if (firstConv.participants.length >= 2) {
            const lecturerParticipant = firstConv.participants.find(
              (p) => p.user?.role === 'LECTURER'
            );
            if (lecturerParticipant?.user?.id) {
              setCurrentUserId(lecturerParticipant.user.id);
            }
          }
        }
      })
  ...
  useEffect(() => {
    ...
    lecturerApi.conversations.contacts(contactQuery || undefined, contactRole || undefined)
      .then(setContacts)
      .catch(() => undefined)
